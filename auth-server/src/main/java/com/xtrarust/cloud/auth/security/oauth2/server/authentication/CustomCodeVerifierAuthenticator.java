...
/**
 * 自定义{@code CodeVerifierAuthenticator}<br>
 *
 * @author gova
 */
public final class CustomCodeVerifierAuthenticator {
    ...
    private boolean authenticate(OAuth2ClientAuthenticationToken clientAuthentication,
        ...
        if (!authorizationCodeGrant(parameters) && !refreshTokenGrant(parameters)) {
            return false;
        }
        ...
        if (authorizationCodeGrant(parameters)) {
            ...
            authorization = this.authorizationService
                    .findByToken((String) parameters.get(OAuth2ParameterNames.REFRESH_TOKEN), OAuth2TokenType.REFRESH_TOKEN);
        }
    ...
    private static boolean refreshTokenGrant(Map<String, Object> parameters) {
